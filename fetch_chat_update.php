<?php
    ...
    if(isset($_SESSION['last_chat'])){
        $lc=$_SESSION['last_chat'];
        $msg_query="SELECT chat_id,from_id,to_id,content FROM mrinal_chats WHERE ((from_id=$my_id AND to_id=$other_id) OR (from_id=$other_id AND to_id = $my_id)) AND chat_id>$lc ORDER BY sent_time ASC;";
